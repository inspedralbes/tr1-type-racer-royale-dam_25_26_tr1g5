...
// Abans venia de localStorage.userName (login fake). Ara és totalment local:
const myNickname = ref('Tu')
...
// --- Només lògica local (sense backend ni token) ---
onMounted(() => {
  // 1. Comprovem el rècord de repeticions (local, no depèn de login)
  ...
  // 2. Rànquing local (de moment només "Tu")
  //    Si més endavant vols ranking de grup real, aquí és on tornaries a cridar l'API.
  if (isGroupSession.value) {
    classificacio.value = [
      {
        nom: myNickname.value,
        reps: reps.value,
        series: series.value
      }
    ]
...
const posicioUsuari = computed(() => {
  // Utilitzem la classificació ja neta i ordenada (en local)
  return classificacio.value.findIndex(e => e.nom === myNickname.value) + 1
})
        ...
        <v-row justify="center">
          <v-col cols="12" md="10" lg="8">
            <v-card class="pa-6 text-center" elevation="3">
              ...
              <!-- Bloc de rànquing: ara mateix també funciona OFFLINE -->
                ...
                <v-table>
                  ...
                  <tbody>
                    ...
                    <tr
                      ...
                      :key="index"
                      :class="{ 'bg-orange-lighten-5 font-weight-bold': row.nom === myNickname }"
                    >
                      <td>#{{ index + 1 }}</td>
                      <td>{{ row.nom }}</td>
                      <td>{{ row.series }}</td>
